...
#   dplyr::full_join(., dplyr::filter(Div_ind_full_long, 
#                                     Country %in% name_country_full), 
#                    by = 'Year') %>%
#   dplyr::select(c(Year, Country, Div_Index, everything())) %>%
#   dplyr::arrange(Country)

panel_common <- Div_ind_full_long_2 %>%
  dplyr::filter(Country %in% name_country_full) %>%
  dplyr::left_join(., RHS_common, by = 'Year') %>%
  dplyr::select(c(Year, Country, Div_Index, everything())) %>%
  dplyr::arrange(Country)
...
nest_panel_common <- nest_panel_common %>%
  dplyr::filter(Div_NA_fraction < 0.8) %>%
  dplyr::mutate('summary_OLS' = purrr::map(data, func_div_ols))
...
names(OLS_full_print) <- nest_panel_common$Country
...
form_common_2 <- Div ~ TED + VIX + SENT + FEDFUNDS + INTERNET + EZ
