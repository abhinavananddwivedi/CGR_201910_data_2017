...
Fig_1_trend_equity <- plot(x_reg, y_reg, type = 'l', xlab = 'Years', ylab = 'Diversification') 
                      grid() 
...
world_trend <- data.frame('World' = summary(lm(y_reg ~ x_reg))$coefficients['x_reg',])
...
print_table_2 <- sapply(c(world_trend, print_trend), rbind) %>% t()

print_table_2 <- data.frame(print_table_2) %>%
  tibble::rownames_to_column(., var = 'Country') %>%
  tibble::as_tibble() 

names(print_table_2) <- c('Country', 'Estimate', 'Std error', 'T stats', 'p value')
...
# print_trend_P1 <- sapply(print_trend_P1, rbind) %>% t()
...
print_table_7_OLS <- data.frame(OLS_dev_emerg_frontier) %>%
...
name_country_table_7_12 <- c('Australia', 'Belgium', 'Canada', 'France', 
                             'Germany', 'Italy', 'Japan', 'Netherlands', 
                             'New Zealand', 'South Africa', 'UK', 'US')

OLS_12 <- nest_panel_common_ols %>%
  dplyr::filter(Country %in% name_country_table_7_12) %>%
  dplyr::select(Country, summary_OLS)

print_table_7_OLS_12 <- purrr::map(OLS_12$summary_OLS,
                                   function(ols_summary){return(ols_summary$coefficients)})
names(print_table_7_OLS_12) <- OLS_12$Country
...
panel_common_ols_2_dev <- func_delta_int(panel_common_dev)
panel_common_ols_2_emerg <- func_delta_int(panel_common_emerg_alt)
panel_common_ols_2_frontier <- func_delta_int(panel_common_frontier)

OLS_full_2 <- func_div_ols(panel_common_ols_2, form_delta_internet)
OLS_full_2_dev <- func_div_ols(panel_common_ols_2_dev, form_delta_internet)
OLS_full_2_emerg <- func_div_ols(panel_common_ols_2_emerg, form_delta_internet)
OLS_full_2_frontier <- func_div_ols(panel_common_ols_2_frontier, form_delta_internet)
...
# Full
data_ols_full <- panel_common_2 %>%
  dplyr::rename('Div' = Div_Index)

table_7_ols_full <- plm::plm(data = data_ols_full, formula = form_common,
                        model = 'pooling') %>% summary()

print_table_7_full <- data.frame(table_7_ols_full$coefficients) %>%
...
panel_est_common_idio_country_internet_emerg <- func_panel_est(formula = form_common_idio_country_internet, 
                                                             panel_data = dplyr::filter(panel_common_idio,
                                                                                        Country %in% name_country_emerging_alt))

# Frontier countries only
panel_est_common_idio_country_internet_frontier <- func_panel_est(formula = form_common_idio_country_internet, 
                                                             panel_data = dplyr::filter(panel_common_idio,
                                                                                        Country %in% name_country_frontier))
...
# Developed
panel_est_com_idio_pc1_dev_bal <- func_panel_est(form_common_idio_dev_pc1,
                                                 dplyr::filter(panel_common_idio_balanced,
                                                               Country %in% name_country_developed))
# Emerging
panel_est_com_idio_pc1_emerg_bal <- func_panel_est(form_common_idio_dev_pc1,
                                                   dplyr::filter(panel_common_idio_balanced,
                                                                 Country %in% name_country_emerging_alt))
# Frontier---NO FRONTIER COUNTRY IN BALANCED
# panel_est_com_idio_pc1_frontier_bal <- func_panel_est(form_common_idio_dev_pc1,
#                                                  dplyr::filter(panel_common_idio_balanced,
#                                                                Country %in% name_country_frontier))
...
# Pre 74 cohort
Div_ind_country_73 <- Div_ind_full_wide %>%
  dplyr::select(Year, dplyr::all_of(country_73))
Div_mean_73 <- apply(Div_ind_country_73[, -1], 1, function(vec){return(mean(vec, na.rm = T))})
Div_ind_country_73 <- Div_ind_country_73 %>%
  tibble::add_column('Div_mean_73' = Div_mean_73) %>%
  dplyr::select(Year, Div_mean_73, everything())

# 74-83 cohort
Div_ind_country_83 <- Div_ind_full_wide %>%
  dplyr::select(Year, dplyr::all_of(country_83))
Div_mean_83 <- apply(Div_ind_country_83[, -1], 1, function(vec){return(mean(vec, na.rm = T))})
Div_ind_country_83 <- Div_ind_country_83 %>%
  tibble::add_column('Div_mean_83' = Div_mean_83) %>%
  dplyr::select(Year, Div_mean_83, everything())

# 84-93 cohort
Div_ind_country_93 <- Div_ind_full_wide %>%
  dplyr::select(Year, dplyr::all_of(country_93))
Div_mean_93 <- apply(Div_ind_country_93[, -1], 1, function(vec){return(mean(vec, na.rm = T))})
Div_ind_country_93 <- Div_ind_country_93 %>%
  tibble::add_column('Div_mean_93' = Div_mean_93) %>%
  dplyr::select(Year, Div_mean_93, everything())
...
Div_mean_cohort <- Div_ind_country_73 %>%
  dplyr::select(Year, Div_mean_73) %>%
  tibble::add_column('Div_mean_83' = Div_ind_country_83$Div_mean_83,
                     'Div_mean_93' = Div_ind_country_93$Div_mean_93,
