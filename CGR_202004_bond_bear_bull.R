...
source('CGR_202003_bond_alt.R', echo = F)
...
func_div_pre_cohort <- function(nest_data = nest_year_lhs_bear)
{
  ...
  nest_year_pre_cohort_b_final <- nest_year_pre_cohort_b_final %>%
  ...
  nest_year_pre_cohort_b_final_3 <- nest_year_pre_cohort_b_final %>%
    dplyr::filter(length_list_RHS != length_list_lag_eig) %>%
  ...
  nest_year_pre_cohort_b_regress <- rbind(nest_year_pre_cohort_b_final_3,
                                        nest_year_pre_cohort_b_final_2) %>%
    dplyr::arrange(Year)
  ...
  nest_year_pre_cohort_b_regress <- nest_year_pre_cohort_b_regress %>%
    dplyr::mutate('div_pre_cohort_name' = purrr::map2(div_pre_cohort, lhs_pre_cohort,
