...
name_country <- colnames(df_bond)[-c(1,2,3,4)] #country names
num_country <- length(name_country)

name_country_pre86 <- df_bond %>% 
...
year_min <- min(df_bond$Year) #earliest year
year_max <- max(df_bond$Year) #latest year
num_years <- year_max - year_min + 1
year_grid <- year_min:year_max
date_grid <- df_bond$Date[-1]
...
df_log_pr <- apply(df_bond[,-c(1,2,3,4)], 2, func_log_ret) %>%
  tibble::as_tibble(.) 
...
# With log returns
df_bond_clean <- cbind('Date' = date_grid, 
                         'Year' = df_bond$Year[-1], 
                         'Month' = df_bond$Month[-1],
                         'Day' = df_bond$Day[-1],
                         df_log_pr) %>% 
  tibble::as_tibble(.) 

# Nesting
nest_df_bond <- df_bond_clean %>%
  dplyr::group_by(Year) %>% 
  dplyr::select(-Date) %>%
  tidyr::nest(.)
...
nest_df_bond_RHS <- df_bond_clean %>% 
...
lag_data_RHS <- c(list(NULL), nest_df_bond_RHS$data[1:(num_years - 1)])
...
RHS_data_matrix_list <- purrr::map2(nest_df_bond_RHS$data[-1],
                                    nest_df_bond_RHS$Lag_data[-1],
                                    func_join_RHS_lag)
...
nest_df_bond_merge <- nest_df_bond_merge %>%
...
lag_eigvec <- nest_df_bond_merge$Eigen_vectors[1:num_years-1]
...
nest_df_bond_LHS_regular <- nest_df_bond_merge %>%
...
nest_df_bond_RHS_pre86 <- nest_df_bond_RHS %>%
...
nest_df_bond_LHS_pre86 <- nest_df_bond_LHS %>%
  dplyr::filter(Year != 1973) %>%
  dplyr::mutate('LHS_country_data_pre86' = purrr::map(LHS_country_data,
                                                      func_select_country_pre86))

nest_df_bond_merge_pre86 <- dplyr::left_join(list_unnest_rhs_pre86,
                                               nest_df_bond_LHS_pre86,
                                               by = 'Year') %>%
